...
Practical tips:
- Be explicit about format, length, and constraints — models default to a reasonable guess when instructions are vague.
...
5. **Re-ranking** (optional) — use a smaller, more precise model to re-score retrieved chunks.
...
- Clustering, Recommendation systems, Deduplication, RAG retrieval step.

Practical note: embedding model choice matters — a model trained on general web text may underperform on code or domain-specific text.
...
Evaluation is the hard part: agents fail in ways that are hard to catch with simple accuracy metrics. Logging every step (not just final output) is essential for debugging.
...
- **Fine-tuning last** — if the problem is the model knows the facts but doesn’t behave/format/reason the way you need, and you have enough quality training examples.

Approaches:
- **Full fine-tuning** — update all model weights. Expensive, needs a lot of data and compute.
- **LoRA** (Low-Rank Adaptation) — freeze the base model, train small low-rank adapter matrices. Much cheaper, easy to swap adapters per task.
- **QLoRA** — LoRA combined with a quantized (lower-precision) base model, cutting memory requirements further.

Risks: Catastrophic forgetting, overfitting on small datasets, data quality matters more than data quantity.

---

## 7. Vector Databases

Purpose: store embeddings and support fast approximate nearest-neighbor (ANN) search at scale.
...
- **IVF** (Inverted File Index) — clusters vectors, searches only the most relevant clusters.

Examples: FAISS (library), Chroma (lightweight, good for prototyping), Weaviate, Milvus, Pinecone (managed/hosted).

Things to actually check before picking one:
- Does it support metadata filtering alongside vector search?
- How does it handle updates/deletes?
- Hosting cost and latency at the scale you actually need.

---

*Next to explore: evaluation frameworks for RAG/agents, multimodal embeddings, structured generation/constrained decoding.*
